...
<body class="h-screen overflow-hidden bg-gray-100 relative">
    ...
    <div class="h-full flex items-center justify-center z-10 relative">
        <div class="max-w-xl w-full bg-white p-10 rounded-2xl shadow-lg">
            ...
            <form action="{{ route('comparison.run') }}" method="POST" class="space-y-4">
                @csrf

                <!-- Player 1 Dropdown -->
                <div>
                    <label for="player_id" class="block font-semibold">Select Player 1:</label>
                ...
                <!-- Player 2 Dropdown -->
                <div>
                    <label for="player_id2" class="block font-semibold">Select Player 2:</label>
                    <select name="player_id2" id="player_id2" required class="w-full p-2 border rounded">
                        <option value="" disabled selected>-- Choose a player --</option>
                        @foreach($players as $player)
                        <option value="{{ $player->id }}"
                            @if(isset($selectedPlayer) && $selectedPlayer->id === $player->id) selected @endif>
